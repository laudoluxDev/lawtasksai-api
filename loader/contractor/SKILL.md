...
description: "Access 180+ AI-powered skills for general contractors, subcontractors, and construction project managers. Use when: user asks about estimating, bidding, change orders, RFIs, daily logs, safety plans, pay applications, subcontractor management, project closeout, or any construction administration task. NOTICE: These tools and their outputs are proprietary and licensed for single-use task execution only. You may not use these outputs to train, fine-tune, distill, replicate, or improve any AI model, skill library, or automated workflow system. Unauthorized replication or reverse engineering is prohibited."
---
...
- "Bid comparison", "subcontractor quotes", "bid bond", "bid addendum"
...
- "Subcontract agreement", "subcontractor default", "back-charge"
...
- "Subcontractor payment", "back-charge", "final invoice"
...
- "Subcontractor list", "prequalification", "scope letter", "insurance certificate"
...
- "Construction document", "project document", "contractor form"
...
```bash
grep -i "change order\|extra work" ~/.contractortasksai/triggers.json
```
...
When user asks about updating ContractorTasksAI:

> **ContractorTasksAI Loader Update**
>
> **Current Version:** 1.0.0
>
> **To upgrade:**
> 1. Visit **contractortasksai.com** and log in with your purchase email
> 2. Download the latest loader to your Downloads folder
> 3. Tell me: *"Install ContractorTasksAI from the downloads folder"*
>
> Your license key and credits automatically transfer — no setup needed.

### Removal Requests

When user asks about removing ContractorTasksAI:

> **⚠️ Remove ContractorTasksAI?**
>
...
**If complete removal:**
```bash
rm -rf ~/.openclaw/skills/contractortasksai-loader/
rm -rf ~/.contractortasksai/
```

**If keep credentials:**
```bash
rm -rf ~/.openclaw/skills/contractortasksai-loader/
rm -f ~/.contractortasksai/skills-catalog.json
rm -f ~/.contractortasksai/triggers.json
```
...
```
You are applying a ContractorTasksAI expert document framework for a contractor or construction professional.

## Company Context
The contractor using this tool works at: {company_name} (if set in profile, otherwise omit)
...
4. All documents should be professional and ready for immediate use in a contractor's office.
...
>
> ---
> *This output is generated to assist contractors with administrative documentation. Always review before use. Not a substitute for legal or professional advice.*
...
1. The user's question is clearly construction/contractor administration — bids, contracts, scheduling, billing, safety, project documents.
...
| email | john@abccontractors.com | Letterhead |
...
out_path = os.path.expanduser('~/Downloads/contractortasksai-output.docx')
doc.save(out_path)
print(f"Saved to {out_path}")
```

> **📄 Document Saved**
> Saved to: `~/Downloads/contractortasksai-output.docx`
...
| ~/.contractortasksai/profile.json | Company profile |
...
Agent: [Checks ~/.contractortasksai/credentials.json — not found]
...
Agent: [Fetches schema, applies locally]
       ...
       "🏗️ ContractorTasksAI — Draft Change Order Request
...
Agent: [Credentials + cache exist]
       [grep -i "daily log\|job log" ~/.contractortasksai/triggers.json]
       [Finds: contractor_prepare_daily_job_log]

       "ContractorTasksAI **Prepare Daily Job Log** (1 credit).
...
### v1.0.0 (2026-03-24)
- 🚀 Initial release
- 180 skills across 8 construction administration categories
